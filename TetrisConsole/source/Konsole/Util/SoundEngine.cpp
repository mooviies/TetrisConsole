...
static ma_result embeddedVFS_onOpen(ma_vfs *pVFS, const char *pFilePath, const ma_uint32 openMode, ma_vfs_file *pFile) {
    ...
    const unsigned char *data;
    size_t size;
    if (!findEmbeddedMedia(pFilePath, &data, &size)) return MA_DOES_NOT_EXIST;
...
static ma_result embeddedVFS_onOpenW(ma_vfs *pVFS, const wchar_t *pFilePath, const ma_uint32 openMode,
                                     ma_vfs_file *pFile) {
    (void)pVFS;
    (void)pFilePath;
    (void)openMode;
...
static ma_result embeddedVFS_onRead(ma_vfs *pVFS, const ma_vfs_file file, void *pDst, const size_t sizeInBytes,
                                    size_t *pBytesRead) {
    (void)pVFS;
...
// NOLINTBEGIN(readability-non-const-parameter) - signature required by miniaudio VFS interface
static ma_result embeddedVFS_onWrite(ma_vfs *pVFS, const ma_vfs_file file, const void *pSrc, const size_t sizeInBytes,
...
static ma_result embeddedVFS_onSeek(ma_vfs *pVFS, const ma_vfs_file file, const ma_int64 offset,
    ...
    if (newCursor < 0 || static_cast<size_t>(newCursor) > ef->size) return MA_INVALID_ARGS;
...
static MaSoundPtr createStreamSound(ma_engine *engine, const char *file, const bool looping) {
    ...
    if (const ma_result result = ma_sound_init_from_file(engine, file, MA_SOUND_FLAG_STREAM, nullptr, nullptr, raw);
        result != MA_SUCCESS) {
        cerr << "miniaudio error loading " << file << " (" << result << ")" << endl;
        delete raw;
        return nullptr;
    }
    if (looping) ma_sound_set_looping(raw, MA_TRUE);
    return MaSoundPtr(raw);
}

static MaSoundPtr createEffectSound(ma_engine *engine, const char *file) {
    auto *raw = new ma_sound();
    if (const ma_result result = ma_sound_init_from_file(engine, file, MA_SOUND_FLAG_DECODE, nullptr, nullptr, raw);
        result != MA_SUCCESS) {
        cerr << "miniaudio error loading " << file << " (" << result << ")" << endl;
        delete raw;
        return nullptr;
...
void SoundEngine::playMusic(const string &name) {
    ...
    // Stop current music
    if (s_musicPlaying != nullptr) ma_sound_stop(s_musicPlaying);
...
void SoundEngine::unpauseMusic() {
    if (s_musicPlaying != nullptr) ma_sound_start(s_musicPlaying);
}
...
void SoundEngine::setMusicVolume(const float volume) {
    ...
    if (s_musicPlaying != nullptr) ma_sound_set_volume(s_musicPlaying, volume);
}
