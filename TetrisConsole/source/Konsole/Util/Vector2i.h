...
struct Vector2i {
    ...
    Vector2i operator+(const Vector2i &right) const { return {row + right.row, column + right.column}; }

    Vector2i operator-(const Vector2i &right) const { return {row - right.row, column - right.column}; }

    bool operator==(const Vector2i &right) const { return row == right.row && column == right.column; }
