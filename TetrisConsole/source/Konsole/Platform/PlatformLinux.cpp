...
int Platform::getKey() {
    unsigned char c;
    if (read(STDIN_FILENO, &c, 1) != 1) return 0; // KEY_ESCAPE as fallback
...
int Platform::getKeyTimeout(int timeoutMs) {
    ...
    if (::select(STDIN_FILENO + 1, &fds, nullptr, nullptr, &tv) > 0) return getKey();
    return -1;
}

bool Platform::wasResized() {
    if (!s_resized) return false;
