...
HighScoreDisplay::HighScoreDisplay() : _leftPanel(kLeftInterior), _rightPanel(kRightInterior) {
    ...
    _tabRow = _leftPanel.addRow(
        {Cell("Marathon", Align::Center), Cell("Sprint", Align::Center), Cell("Ultra", Align::Center)});
    ...
    _lvlStatRow = _rightPanel.addRow({Cell("Level", Align::Left, 15, kLabelWidth), Cell("", Align::Center)});
    ...
    _rightPanel.addSeparator();
    _startRow = _rightPanel.addRow({Cell("FirstLvl", Align::Left, 15, kLabelWidth), Cell("", Align::Center)});
    _previewRow = _rightPanel.addRow({Cell("Preview", Align::Left, 15, kLabelWidth), Cell("", Align::Center)});
    _ghostRow = _rightPanel.addRow({Cell("Ghost", Align::Left, 15, kLabelWidth), Cell("", Align::Center)});
    _holdRow = _rightPanel.addRow({Cell("Hold", Align::Left, 15, kLabelWidth), Cell("", Align::Center)});
    _modeRow = _rightPanel.addRow({Cell("LckDwn", Align::Left, 15, kLabelWidth), Cell("", Align::Center)});
...
void HighScoreDisplay::updateTabRow() {
    for (size_t i = 0; i < VARIANT_COUNT; i++)
        _leftPanel.setCellColor(_tabRow, i, static_cast<size_t>(_activeTab) == i ? rlutil::YELLOW : Color::GREY);
}
...
void HighScoreDisplay::updateDisplay(const vector<HighScoreRecord> &hs) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (i < static_cast<int>(hs.size())) {
            ...
            _leftPanel.setCell(_listRows[static_cast<size_t>(i)], 0, prefix + rank + "----------" + " " + "----------");
        }
    }
    ...
    if (_selected < static_cast<int>(hs.size())) {
        ...
        if (rec.mode == LockDownMode::Classic)
            modeStr = "Classic";
        else if (rec.mode == LockDownMode::ExtendedInfinity)
            modeStr = "Infinite";
...
void HighScoreDisplay::open(const HighScoreTable &allHighscores, GameVariant initialVariant) {
    ...
    while (true) {
        ...
        if (Platform::wasResized()) {
            if (!Platform::isTerminalTooSmall()) {
                if (Menu::onResize) Menu::onResize();
                reposition();
                _leftPanel.invalidate();
                _rightPanel.invalidate();
            }
        ...
        if (Menu::shouldExitGame && Menu::shouldExitGame()) break;
    }
...
static vector<Rect> buildExclusionZones(int ox, int oy, const Panel &left, const Panel &right) {
    return {{1 + ox, 1 + oy, kWindowWidth, kTitleHeight},
            {left.x(), left.y(), left.width(), left.height()},
            {right.x(), right.y(), right.width(), right.height()}};
}

string HighScoreDisplay::openForNewEntry(const HighScoreTable &allHighscores, const HighScoreRecord &newRecord,
                                         GameVariant variant) {
