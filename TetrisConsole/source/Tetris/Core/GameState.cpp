...
static void sortAndCap(vector<HighScoreRecord>& hs) {
	...
	if (hs.size() > kMaxHighscores)
		hs.resize(kMaxHighscores);
}
...
void GameState::loadHighscore()
{
	...
	if (version == 3) {
		...
		for (size_t v = 0; v < VARIANT_COUNT; v++) {
			...
			size_t bucket = clamp(static_cast<size_t>(variantId),
			                      static_cast<size_t>(0), VARIANT_COUNT - 1);
			for (uint32_t i = 0; i < count; i++) {
				if (!in) break;
...
void GameState::saveHighscore() {
	if (stats.hasBetterHighscore) {
		...
		if (bucket.size() > kMaxHighscores)
			bucket.resize(kMaxHighscores);
	}
...
void GameState::activateHighscore() {
	...
	stats.highscore = (bucket.size() >= kMaxHighscores)
		? bucket[kMaxHighscores - 1].score : 0;
}
