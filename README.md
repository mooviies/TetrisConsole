...
A cross-platform Tetrominos game that runs in the terminal, written in C++17. Features modern Guideline mechanics including SRS wall kicks, T-spins, hold piece, back-to-back bonuses, and three game modes.
...
**Variant** — Game mode:
- **Marathon** (default) — Classic mode. Level up every time you clear the goal, scoring as much as possible.
- **Sprint** — Clear 40 lines as fast as you can.
- **Ultra** — Score as many points as possible in 2 minutes.

## Game Modes

**Marathon** — The classic experience. Clear lines to advance through levels, with gravity increasing at each level. The goal increases per level (level × 5 lines). Your score is the primary measure of success.
...
- Back-to-back bonus (50% extra) for consecutive Quads or T-spins
- Combo (Ren) tracking with text notifications for impressive moves
- 15 levels with Guideline gravity speeds
- In-game stats: score, time, TPM, LPM, level, lines, goal, Quads, combos, T-spins
...
All base points are multiplied by the current level. Consecutive Quads or T-spin line clears earn a 1.5x back-to-back bonus.
