...
- **Tetrominos** — Facade that owns the above three plus `Menu&` references and `HighScoreDisplay&`. Dispatches `StepResult`, handles pause/game-over flow, cycles music. `step()` takes an `InputSnapshot` parameter.
...
- **Core/** — `TetrominosConsole.cpp` (entry point, main loop), `Tetrominos` (facade), `GameController` (phase orchestrator), `PieceMovement` (falling-phase logic), `LineClear` (line-clear pipeline), `GameState` (model), `GameRenderer` (view), `GameTypes.h` (`LineClearState`, `HardDropTrail`), `Constants.h` (includes `VARIANT` enum, `VARIANT_COUNT`), `InputSnapshot.h` (defines `Action` enum and `InputSnapshot` struct).
...
- `_shouldExit` is set via `Tetrominos::exit()` → `GameState::setShouldExit(true)`; the game exits from the main loop in `TetrominosConsole.cpp`.
