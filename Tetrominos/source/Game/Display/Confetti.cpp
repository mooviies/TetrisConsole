...
static constexpr int kColors[] = {Color::LIGHTBLUE, Color::BLUE,    Color::BROWN, Color::YELLOW,
                              Color::GREEN,     Color::MAGENTA, Color::RED};
static constexpr int kColorCount = 7;

void Confetti::start(int screenW, int screenH, int offsetX, int offsetY, vector<Rect> exclusionZones) {
    _screenW = screenW;
    _screenH = screenH;
    _offsetX = offsetX;
    _offsetY = offsetY;
...
void Confetti::update() {
    ...
    const float elapsed = chrono::duration<float>(now - _lastUpdate).count();
    _lastUpdate = now;

    for (auto &p : _particles) {
        p.y += p.speed * elapsed;
        const int newRow = static_cast<int>(p.y);
...
void Confetti::spawnParticle(Particle &p, bool randomY) const {
    const int slots = _screenW / 2;
    const int slot = Random::getInteger(0, slots - 1);
...
bool Confetti::overlapsExclusion(int px, int py) const {
    return std::any_of(_exclusions.begin(), _exclusions.end(), [px, py](const Rect &r) {
        return py >= r.y && py < r.y + r.h && px + 1 >= r.x && px < r.x + r.w;
    });
}
